    ...
    /* ==========================================================
    FILTER: TAHUN & BULAN (mode cepat) atau RENTANG TANGGAL
    ========================================================== */
    ...
    // Rentang tanggal
    $dari    = isset($_GET['dari'])    && preg_match('/^\d{4}-\d{2}-\d{2}$/', $_GET['dari'])    ? $_GET['dari']    : '';
    $sampai  = isset($_GET['sampai'])  && preg_match('/^\d{4}-\d{2}-\d{2}$/', $_GET['sampai'])  ? $_GET['sampai']  : '';
    $modeRentang = ($dari !== '' && $sampai !== '' && $dari <= $sampai);

    // Daftar tahun
    $tahunList = [];
    $resTahun  = $koneksi->query("SELECT DISTINCT YEAR(tanggal) AS thn FROM kas_koperasi ORDER BY thn DESC");
    if ($resTahun) {
        while ($row = $resTahun->fetch_assoc()) $tahunList[] = (int) $row['thn'];
    }
    if (!in_array($tahunSekarang, $tahunList, true)) array_unshift($tahunList, $tahunSekarang);
    if (!in_array($tahunDipilih,  $tahunList, true)) $tahunDipilih = $tahunSekarang;

    /* ==========================================================
    SALDO AWAL — transaksi SEBELUM periode tampil
    ========================================================== */
    $saldoAwal = 0.0;
    if ($modeRentang) {
        $stmtAwal = $koneksi->prepare(
            "SELECT COALESCE(SUM(CASE WHEN jenis='masuk' THEN nominal ELSE 0 END),0) AS total_masuk,
                    COALESCE(SUM(CASE WHEN jenis='keluar' THEN nominal ELSE 0 END),0) AS total_keluar
             FROM kas_koperasi WHERE tanggal < ?"
        );
        $stmtAwal->bind_param('s', $dari);
    } elseif ($bulanDipilih > 0) {
        $batasTgl = sprintf('%04d-%02d-01', $tahunDipilih, $bulanDipilih);
        $stmtAwal = $koneksi->prepare(
            "SELECT COALESCE(SUM(CASE WHEN jenis='masuk' THEN nominal ELSE 0 END),0) AS total_masuk,
                    COALESCE(SUM(CASE WHEN jenis='keluar' THEN nominal ELSE 0 END),0) AS total_keluar
             FROM kas_koperasi WHERE tanggal < ?"
        );
        $stmtAwal->bind_param('s', $batasTgl);
    } else {
        $stmtAwal = $koneksi->prepare(
            "SELECT COALESCE(SUM(CASE WHEN jenis='masuk' THEN nominal ELSE 0 END),0) AS total_masuk,
                    COALESCE(SUM(CASE WHEN jenis='keluar' THEN nominal ELSE 0 END),0) AS total_keluar
             FROM kas_koperasi WHERE YEAR(tanggal) < ?"
        );
    ...
    /* ==========================================================
    DATA TRANSAKSI PERIODE TERPILIH
    ========================================================== */
    $transaksi = [];
    if ($modeRentang) {
        // Sampai akhir hari sampai (inklusif)
        $sampaiAkhir = $sampai . ' 23:59:59';
    ...
    foreach ($transaksi as $t) {
        ...
        if ($t['jenis'] === 'masuk') { $saldoBerjalan += $nominal; $totalMasuk  += $nominal; }
        else                         { $saldoBerjalan -= $nominal; $totalKeluar += $nominal; }
        $rows[] = [
            'id' => $t['id'], 'tanggal' => $t['tanggal'],
            'keterangan' => $t['keterangan'], 'jenis' => $t['jenis'],
            'nominal' => $nominal, 'saldo' => $saldoBerjalan,
        ];
    }
    ...
    // Label periode
    if ($modeRentang) {
        $fmtDari   = date('d M Y', strtotime($dari));
        $fmtSampai = date('d M Y', strtotime($sampai));
        $labelPeriode = "$fmtDari – $fmtSampai";
    } elseif ($bulanDipilih > 0) {
        $labelPeriode = $namaBulan[$bulanDipilih] . ' ' . $tahunDipilih;
    } else {
        $labelPeriode = (string) $tahunDipilih;
    }
        ...
        <div class="container">
            <div class="page-header">
                ...
                <div class="header-actions no-print">
                    ...
                    <div class="filter-row">
                        ...
                        <select id="bulanFilter" onchange="gantiBulan(this.value)">
                            <option value="0" <?= $bulanDipilih === 0 ? 'selected' : '' ?>>Semua Bulan</option>
                            <?php foreach ($namaBulan as $num => $nama): ?>
                                <option value="<?= $num ?>" <?= $bulanDipilih === $num ? 'selected' : '' ?>><?= $nama ?></option>
                            <?php endforeach; ?>
                        </select>
                        <?php if ($userRole === 'bendahara'): ?>
                        <button type="button" class="btn btn-primary" onclick="bukaModalTambah()">+ Tambah</button>
                        <?php endif; ?>
                    </div>
                    <!-- Baris 2: Rentang Tanggal -->
                    <div class="filter-row filter-rentang<?= $modeRentang ? ' aktif' : '' ?>">
                        <input type="date" id="inputDari" value="<?= htmlspecialchars($dari) ?>" max="<?= date('Y-m-d') ?>">
                        <span class="filter-sep">&mdash;</span>
                        <input type="date" id="inputSampai" value="<?= htmlspecialchars($sampai) ?>" max="<?= date('Y-m-d') ?>">
                        <button type="button" class="btn btn-filter-apply" onclick="terapkanRentang()">Terapkan</button>
                        <?php if ($modeRentang): ?>
                        <button type="button" class="btn btn-filter-reset" onclick="resetRentang()">✕ Reset</button>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="summary-grid">
                <div class="card in">
                    ...
                    <div class="card-body">
                        <div class="label">Total Pemasukan <?= $labelPeriode ?></div>
                ...
                <div class="card out">
                    <div class="icon-badge">&#8593;</div>
                    <div class="card-body">
                        <div class="label">Total Pengeluaran <?= $labelPeriode ?></div>
                ...
                <div class="card balance">
                    ...
                    <div class="card-body">
                        <div class="label">Saldo Akhir <?= $bulanDipilih > 0 ? $labelPeriode : '' ?></div>
