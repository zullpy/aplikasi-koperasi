...
async function loadData() {
    try {
        const res = await fetch('../database/get-stok-barang.php?_t=' + Date.now());
...
function getStatus(totalQty) {
    ...
    if (totalQty < 5) return { cls: 'st-rendah', label: 'Rendah' };
    if (totalQty < 10) return { cls: 'st-menipis', label: 'Menipis' };
...
function draw() {
    ...
    if (total === 0) {
        tbody.innerHTML = `<tr><td colspan="11" style="text-align:center;padding:32px;color:#64748b;">Tidak ada data barang</td></tr>`;
    } else {
        tbody.innerHTML = slice.map((d, i) => {
            ...
            const tBarangLabel = d.total_barang > 0 ? fmtTotalQty(d, d.total_barang) : null;
            const tAmbilLabel = d.total_pengambilan > 0 ? `- ${fmtTotalQty(d, d.total_pengambilan)}` : null;
            const tSisaLabel = tQty > 0 ? fmtTotalQty(d, tQty) : null;
            ...
            return `<tr>
                ...
                <td class="center">${tSisaLabel ? `<span class="num-stok num-sisa-barang">${tSisaLabel}</span>` : `<span class="num-zero">0</span>`}</td>
...
function drawMobileCards(slice, total) {
    ...
    el.innerHTML = slice.map((d, i) => {
        ...
        const st = getStatus(tQty);
        const no = (currentPage - 1) * PAGE_SIZE + i + 1;
        const tBarangLabel = fmtTotalQty(d, d.total_barang);
        const tAmbilLabel = d.total_pengambilan > 0 ? `- ${fmtTotalQty(d, d.total_pengambilan)}` : '0';
        ...
        return `<div class="mc-item">
            ...
            <div class="mc-stats-grid">
                <div class="mc-stat-item mc-stat-total">
            ...
            <div class="mc-bottom">
                <div class="mc-total-wrap">
                    <span class="mc-total-label">Total Sisa Barang</span>
