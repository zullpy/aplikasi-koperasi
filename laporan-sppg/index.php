...
$bisaTtd       = in_array($role_login_saat_ini, ['admin', 'purchase', 'purchase_stok'], true);
?>
<!DOCTYPE html>
<html lang="id">
                    ...
                    <tbody>
                        ...
                        <?php else: ?>
                            <?php foreach ($rows as $no => $row):
                                ...
                                $ttdSaya  = $ttdMasuk['purchase'] ?? null;
                            ?>
                                <tr class="data-row">
                                    ...
                                    <td class="center">
                                        <span class="saldo-pill <?= $sisa < 0 ? 'saldo-pill--minus' : 'saldo-pill--plus' ?>">
                                            ...
                                            <?= rupiah(abs($sisa)) ?>
                                                                      ...
                                                                      <td class="center">
                                        <?php
                                        $rawBukti = $row['bukti_transfer'] ?? null;
                                        $buktiFiles = [];
                                        if (!empty($rawBukti)) {
                                            if (is_array($rawBukti)) {
                                                $buktiFiles = $rawBukti;
                                            } else {
                                                $decoded = json_decode($rawBukti, true);
                                                if (is_array($decoded)) {
                                                    $buktiFiles = array_values(array_filter($decoded));
                                                } else {
                                                    $buktiFiles = [$rawBukti];
                                                }
                                            }
                                        }
                                        $buktiFiles = array_values(array_filter($buktiFiles));
                                        ?>
                                        <?php if (!empty($buktiFiles)): ?>
                                            <?php $safeBuktiJson = htmlspecialchars(json_encode($buktiFiles), ENT_QUOTES, 'UTF-8'); ?>
                                            <button type="button" class="btn btn--outline"
                                                onclick="bukaBuktiTransfer(<?= $safeBuktiJson ?>)">
                                                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                                    <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" />
                                                    <circle cx="12" cy="12" r="3" />
                                                </svg>
                                                Lihat <?= count($buktiFiles) > 1 ? '(' . count($buktiFiles) . ')' : '' ?>
                                            </button>
                                        <?php else: ?>
                                ...
                                <tr class="detail-row" id="detail<?= $row['id'] ?>">
                                    <td colspan="9">
                                        <div class="detail-inner">
                                            ...
                                            <!-- ─── Status Tanda Tangan Pembuat (Saepul Misbah) ─────────── -->
                                            <div class="detail-inner__title" style="margin-top:18px">
                                                ...
                                                Status Tanda Tangan Pembuat
                                            </div>
                                            <div class="ttd-status-grid">
                                                <?php
                                                    $ttdItem = $ttdMasuk['purchase'] ?? null;
                                                    $signerName = 'Saepul Misbah';
                                                    $roleLabel  = 'Pembuat / Staf Pengadaan';
                                                ?>
...
<div class="modal-overlay" id="modalNotaGaleri" onclick="tutupModal('modalNotaGaleri', event)">
    ...
    <script>
        ...
        function bukaBuktiTransfer(files) {
            if (!files) return;
            let list = [];
            if (Array.isArray(files)) {
                ...
                try {
                    ...
                    list = Array.isArray(parsed) ? parsed : [files];
                } catch (e) {
                    list = [files];
                }
            }
            ...
            const titleEl = document.getElementById('titleNotaGaleri');
            if (titleEl) titleEl.textContent = 'Bukti Transfer (' + list.length + ' Foto)';
            if (grid) {
                grid.innerHTML = list.map(function(file) {
                    ...
                    if (ext === 'pdf') {
                        return '<div class="nota-card-item" onclick="bukaGambar(\'' + path + '\', \'Bukti Transfer (PDF)\', true)" style="cursor:pointer;">' +
                               ...
                               '<div style="font-size:12px;text-align:center;padding:4px;color:var(--primary);font-weight:600;">Lihat PDF</div></div>';
                    } else {
                        return '<div class="nota-card-item" onclick="bukaGambar(\'' + path + '\', \'Bukti Transfer\', false)" style="cursor:pointer;">' +
                               '<img src="' + path + '" alt="Bukti Transfer" style="width:100%;height:140px;object-fit:cover;border-radius:4px;">' +
                               '</div>';
                    }
                }).join('');
            }
