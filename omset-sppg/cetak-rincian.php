<?php
// cetak-rincian.php - Preview & Cetak Rincian Omset SPPG
...
$bulanParam = $_GET['bulan'] ?? null;

if ($startParam && $endParam) {
    $start = $startParam;
    $end   = $endParam;
    $labelPeriode = date('d/m/Y', strtotime($start)) . ' s/d ' . date('d/m/Y', strtotime($end));
    $pdfFilename  = "RINCIAN-OMSET-{$start}-SD-{$end}.pdf";
} elseif ($bulanParam) {
    $start = $bulanParam . '-01';
    $end   = date('Y-m-t', strtotime($start));
    $bulanIndo = match(date('F', strtotime($start))) {
        'January' => 'Januari', 'February' => 'Februari', 'March' => 'Maret',
        'April' => 'April', 'May' => 'Mei', 'June' => 'Juni',
        'July' => 'Juli', 'August' => 'Agustus', 'September' => 'September',
        'October' => 'Oktober', 'November' => 'November', 'December' => 'Desember',
        default => date('F', strtotime($start))
    };
    $labelPeriode = $bulanIndo . ' ' . date('Y', strtotime($start));
    $pdfFilename  = "RINCIAN-OMSET-{$bulanParam}.pdf";
} else {
    $bulanParam = date('Y-m');
    $start = $bulanParam . '-01';
    $end   = date('Y-m-t', strtotime($start));
    $labelPeriode = date('F Y', strtotime($start));
    $pdfFilename  = "RINCIAN-OMSET-{$bulanParam}.pdf";
}
...
$stmt->close();
if (empty($rows)) {
    die('Data omset harian untuk periode tersebut tidak ditemukan');
}
...
foreach ($rows as $r) {
    ...
    $total['anggaran_diterima']   += (float)$r['biaya_admin'];
...
<head>
    ...
    <title>Rincian Omset SPPG - <?= htmlspecialchars($labelPeriode) ?></title>
...
<body>
    ...
    <div class="toolbar">
        <div class="toolbar-left">
            ...
            <div>
                <div class="toolbar-title">Rincian Omset SPPG</div>
    ...
    <div class="preview-wrapper">
        <div class="preview-container" id="pdfContent">
            ...
            <div class="subjudul-laporan">Periode: <?= htmlspecialchars($labelPeriode) ?></div>

            <table class="tabel-data">
                <thead>
                    <tr>
                        <th rowspan="2" class="center" style="width: 8%;">Tanggal</th>
                        <th rowspan="2" class="right" style="width: 8%;">Total Anggaran</th>
                        <th rowspan="2" class="right" style="width: 8%;">Anggaran Diterima</th>
                        <th rowspan="2" class="right" style="width: 8%;">Pajak</th>
                        <th rowspan="2" class="center" style="width: 4%;">KPM</th>
                        <th rowspan="2" class="right" style="width: 8%;">Belanja Foodcost</th>
                        <th colspan="2" class="center" style="width: 14%;">KBUS</th>
                        <th colspan="2" class="center" style="width: 14%;">Yayasan</th>
                        <th colspan="2" class="center" style="width: 14%;">Koperasi</th>
                        <th rowspan="2" class="right" style="width: 10%;">Nominal Management</th>
                    </tr>
                    <tr>
                        <th class="center" style="width: 5%;">Profit Sharing</th>
                        <th class="right" style="width: 9%;">Nominal</th>
                        <th class="center" style="width: 5%;">Profit Sharing</th>
                        <th class="right" style="width: 9%;">Nominal</th>
                        <th class="center" style="width: 5%;">Profit Sharing</th>
                        <th class="right" style="width: 9%;">Nominal</th>
                ...
                <tbody>
                    <?php foreach ($rows as $row): ?>
                        <tr>
                            ...
                            <td class="right"><?= number_format($row['biaya_admin'], 0, ',', '.') ?></td>
                            <td class="right"><?= number_format($row['pajak'] ?? 0, 0, ',', '.') ?></td>
                    ...
                    <tr class="row-total">
                        ...
                        <td class="right"><?= number_format($total['anggaran_diterima'], 0, ',', '.') ?></td>
    ...
    <script>
        const PDF_FILENAME = <?php echo json_encode($pdfFilename); ?>;
